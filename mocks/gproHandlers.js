...
import { GrammarProResponseData } from "./data/gpro_output_high";

export const gproHandlers = [
  // Evaluate (post)
  ...
  // }),
  rest.post("api/edu/grammarpro", (req, res, ctx) => {
    // return res(ctx.status(200), ctx.json(TEST_API_DATA_INCLUDED_SUPRA));
    // return res(ctx.status(200), ctx.json(TEST_API_DATA_INCLUDED_SUPRA_EMPTY));
    // return res(ctx.status(200), ctx.json(TEST_API_DATA_INCLUDED_SUPRA));
    // return res(
    ...
    // );
    return res(ctx.status(200), ctx.json(GrammarProResponseData));
  }),
];
