...
import { setGrammarData } from "redux/features/grammarpro/gproSlices";
...
import { GRAMMARPRO_CONSTANTS_KO } from "../../../../constants/Constants";
import {
  GproState,
  GrammarProDataType,
  ChangeWordType,
...
export const GrammarPro = ({
  ...
  const [selectedData, setSelectedData] = useState<ChangeWordType>();
  ...
  const checkCefr = (cefr: string) => {
    if (cefr.includes("A")) {
      if (cefr.includes("A1")) {
        return {
          description: GRAMMARPRO_CONSTANTS_KO.low,
          percent: 16,
          color: red[5],
        };
      ...
      if (cefr.includes("B1")) {
        return {
          description: GRAMMARPRO_CONSTANTS_KO.mid,
          percent: 48,
          color: red[1],
        };
      ...
      if (cefr.includes("C1")) {
        return {
          description: GRAMMARPRO_CONSTANTS_KO.high,
          percent: 80,
          color: green[3],
        };
        ...
        >
          ...
          >
            <Row>
              ...
              >
                <div>{data.data.total_summary.total_cefr}</div>
              </Col>
              <Col
              ...
              >
                <Space direction="vertical">
                  <div>{`${
                    checkCefr(data.data.total_summary.total_cefr)?.description
                  }`}</div>
                  <div>
                    <Progress
                      percent={
                        checkCefr(data.data.total_summary.total_cefr)?.percent
                      }
                      steps={6}
                      strokeColor={[
                        green[1],
              ...
              >
                <ChartPage
                  data={data.data.total_summary.change_ct_by_level}
                  isLoading={isLoading}
                />
              </Col>
            </Row>
